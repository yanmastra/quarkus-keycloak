...
import io.yanmastra.quarkusBase.quarkusBase.utils.KeyValueCacheUtils;
...
public class AuthenticationService {
    ...
    @ConfigProperty(name = Constant.PROP_SMALLRYE_JWT_ISSUER, defaultValue = "-")
    String issuer;
    @ConfigProperty(name = Constant.PROP_MP_JWT_VERIFY_CLOCK_SKEW, defaultValue = "0")
    int expiration;
    ...
    public Map<String, Object> createAccessToken(UserTokenPayload userTokenPayload, Instant expiredAt, String sessionId, String existingRefreshToken) {
        if (StringUtils.isBlank(tokenEncryptionSecret) || "-".equals(tokenEncryptionSecret)) throw new BadRequestException("Please set property \"" + Constant.PROP_SECURITY_TOKEN_ENCRYPTION_SECRET + "\".");
    ...
    public String getSessionFromRefreshToken(String refreshToken) {
        if (StringUtils.isBlank(tokenEncryptionSecret)) throw new BadRequestException("Please set property \"" + Constant.PROP_SECURITY_TOKEN_ENCRYPTION_SECRET + "\".");
