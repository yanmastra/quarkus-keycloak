...
envsubst  < ${DIR}/infra/docker/run-debug.sh.template > run-debug.sh
chmod +x run-debug.sh
cp ${DIR}/infra/docker/.env .
cp ${DIR}/infra/docker/.env.example .
envsubst < ${DIR}/infra/docker/pom.xml.template > pom.xml
cat ${DIR}/infra/docker/application.properties.template > src/main/resources/application.properties
