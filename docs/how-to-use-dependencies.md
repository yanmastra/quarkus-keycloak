...
4. Also copy this script ``/infra/docker/install-jar.sh`` or ``/infra/docker/install-jar.bat`` to root folder of your project. This file is to install the jar and pom file to the local Maven, so if someone need to run your project, they just need to run ``./install-jar.sh`` to install the dependency instead of cloning this project again, because the jar and pom already included to your project
5. Don't forget to add the dependency to your pom.xml project, see the detail in each dependency folder [``/extensions/``](/extensions)
